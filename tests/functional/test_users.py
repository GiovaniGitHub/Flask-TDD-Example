...
def test_valid_login_logout(test_client, init_database):
    ...
    response = test_client.post('/login',
                                data=dict(email='johndoe@gmail.com', password='p4ssw0rd123'),
                                follow_redirects=True)
    assert response.status_code == 200
    assert b'Thanks for logging in, johndoe@gmail.com!' in response.data
...
def test_invalid_login(test_client, init_database):
    ...
    """
    response = test_client.post('/login',
                                data=dict(email='johndoe@gmail.com', password='FlaskIsNotAwesome'),
                                follow_redirects=True)
    assert response.status_code == 200
...
def test_valid_registration(test_client, init_database):
    ...
    response = test_client.post('/register',
                                ...
                                follow_redirects=True)
    assert response.status_code == 200
    assert b'Thanks for registering, johndoe@yahoo.com!' in response.data
    assert b'Flask User Management' in response.data
    assert b'Logout' in response.data
    assert b'Login' not in response.data
    assert b'Register' not in response.data
...
def test_invalid_registration(test_client, init_database):
    ...
    """
    response = test_client.post('/register',
                                data=dict(email='johndoe@hotmail.com',
                                          password='FlaskIsGreat',
                                          confirm='FlskIsGreat'),   # Does NOT match!
                                follow_redirects=True)
    assert response.status_code == 200
    assert b'Thanks for registering, johndoe@hotmail.com!' not in response.data
